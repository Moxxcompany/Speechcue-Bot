...
class SubscriptionPlans(models.Model):
    ...
    number_of_bulk_call_minutes = models.DecimalField(
        null=True, max_digits=20, decimal_places=6, blank=True
    )
    ...
    single_ivr_minutes = models.DecimalField(
        max_digits=20, decimal_places=6, default=MAX_INFINITY_CONSTANT
    )
...
class VirtualAccountsTable(models.Model):
    user = models.ForeignKey(
        TelegramUser, on_delete=models.CASCADE, related_name="user_accounts"
    )
...
class UserSubscription(models.Model):
    user_id = models.ForeignKey(
        TelegramUser, on_delete=models.CASCADE, related_name="subscription_user_id"
    )
    ...
    bulk_ivr_calls_left = models.DecimalField(
        null=True, max_digits=20, decimal_places=6, blank=True
    )
    ...
    single_ivr_left = models.DecimalField(
        null=True, max_digits=20, decimal_places=6, blank=True
    )
...
class OveragePricingTable(models.Model):
    overage_pricing = models.DecimalField(
        max_digits=20, decimal_places=6, null=True, blank=True
    )
...
class ManageFreePlanSingleIVRCall(models.Model):
    user_id = models.ForeignKey(
        TelegramUser, on_delete=models.DO_NOTHING, related_name="manage_free_plans"
    )
