...
from payment.models import (
    SubscriptionPlans, MainWalletTable, VirtualAccountsTable, UserSubscription,
    OveragePricingTable, ManageFreePlanSingleIVRCall, UserTransactionLogs, TransactionType, PricingUnits
)

@admin.register(SubscriptionPlans)
class SubscriptionPlansAdmin(admin.ModelAdmin):
    list_display = (
        'plan_id', 'name', 'plan_price', 'single_ivr_minutes',
        'number_of_bulk_call_minutes', 'call_transfer',
        'customer_support_level', 'validity_days'
    )
    ...
    fieldsets = (
        ...
        ('Call Details', {
            'fields': ('number_of_bulk_call_minutes', 'single_ivr_minutes', 'call_transfer')
        }),
        ('Support Details', {
...
class MainWalletTableAdmin(admin.ModelAdmin):
    list_display = (
        'currency', 'xpub', 'address', 'virtual_account', 'mnemonic',
        'deposit_address', 'private_key', 'subscription_id', 'fee',
        'gas_price', 'gas_limit'
    )
...
class VirtualAccountsTableAdmin(admin.ModelAdmin):
    list_display = (
        'get_user_id', 'balance', 'currency', 'account_detail',
        'account_id', 'subscription_id', 'deposit_address',
...
class UserSubscriptionAdmin(admin.ModelAdmin):
    ...
    get_user_id.short_description = 'User ID'
    list_display = (
        'get_user_id', 'subscription_status', 'plan_id', 'bulk_ivr_calls_left',
        'single_ivr_left', 'date_of_subscription', 'date_of_expiry',
        'call_transfer', 'auto_renewal'
    )
    list_filter = ('subscription_status', 'plan_id')
    search_fields = ('user_id__user_id', 'subscription_status', 'plan_id__name')
    ...
    fieldsets = (
        ...
        ('Plan Details', {
            'fields': ('plan_id', 'call_transfer', 'bulk_ivr_calls_left', 'single_ivr_left', 'date_of_expiry')
        }),
    )
...
class OveragePricingTableAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ('Overage Pricing', {
            'fields': ['overage_pricing', 'pricing_unit']
        }),
    ]

@admin.register(ManageFreePlanSingleIVRCall)
class ManageFreePlanSingleIVRCallAdmin(admin.ModelAdmin):
    list_display = ('call_id', 'get_user_id', 'call_duration', 'pathway_id', 'call_number', 'call_status')
...
class UserTransactionLogsAdmin(admin.ModelAdmin):
    list_display = ('transaction_id', 'user_id', 'reference', 'transaction_type', 'transaction_date')
