...
def handle_join_channel(message):
    ...
    bot.send_message(user_id, f"{JOIN_CHANNEL_PROMPT[lg]}\n {CHANNEL_LINK}", reply_markup=get_main_menu())
...
def get_user_profile(message):
    ...
    bot.send_message(user_id, f"{USERNAME[lg]} : {username_formated}", parse_mode="Markdownv2" )
    if user.subscription_status == f'{INACTIVE}':
        ...
        plan_msg = (f"{SUBSCRIPTION_PLAN[lg]}: \n"
    ...
    if user.subscription_status == 'active':
        ...
        if additional_minutes_records.exists():
            ...
            if unpaid_minutes_records.exists():
                bot.send_message(user_id, f"{UNPAID_MINUTES_PROMPT[lg]}", reply_markup=get_main_menu())
                return
        user_data[user_id] = {'step': 'get_batch_numbers'}
        view_flows(message)
    else:
        bot.send_message(user_id, f"{BULK_IVR_SUBSCRIPTION_PROMPT[lg]}", reply_markup=get_subscription_activation_markup())
...
def handle_view_subscription(call):
    ...
    try:
        ...
        if subscription_plan.call_transfer:
            plan_details+=(
                f"{FULL_NODE_ACCESS[lg]} : {CALL_TRANSFER_INCLUDED[lg]}"
            )
        else:
            plan_details+=(
                f"{FULL_NODE_ACCESS[lg]} : {CALL_TRANSFER_EXCLUDED[lg]}"
            )

        bot.send_message(user_id, f"{ACTIVE_SUBSCRIPTION_PLAN_PROMPT[lg]}\n\n{plan_details}",
                         reply_markup=get_billing_and_subscription_keyboard())

    except TelegramUser.DoesNotExist:
        bot.send_message(user_id, f"{SUBSCRIPTION_PLAN_NOT_FOUND[lg]}",
                         reply_markup=get_billing_and_subscription_keyboard())
...
def check_wallet(call):
    user_id = call.message.chat.id
    lg = get_user_language(user_id)
    bot.send_message(user_id, escape_markdown(f"{CHECKING_WALLETS[lg]}"), parse_mode='MarkdownV2')
    try:
        wallet = check_user_balance(user_id)
        ...
        bot.send_message(user_id, f"{WALLET_BALANCE[lg]}\n{balance} {currency}",
                         reply_markup=markup)
    except Exception as e:
        bot.send_message(user_id, escape_markdown(f"{PROCESSING_ERROR[lg]}\n\n{str(e)}"), parse_mode='MarkdownV2')
    return
...
def trigger_single_ivr_call(message):
    ...
    if user.subscription_status == 'active':
        ...
        bot.send_message(user_id, f"{ACTIVATE_SUBSCRIPTION[lg]}",
    ...
    if not valid:
        if missing_sources:
            bot.send_message(chat_id,
                             f"{OUTGOING_CONNECTIONS_MISSING[lg]}"
...
def trigger_add_another_node(message):
    ...
    bot.send_message(message.chat.id, f"{NODE_TYPE_SELECTION_PROMPT[lg]}", reply_markup=keyboard)
...
def handle_call_selection_variable(call):
    ...
    try:
        ...
        if variables is None:
            ...
            bot.send_message(user_id, escape_markdown(msg),
...
def view_main_menu(message):
    ...
    bot.send_message(user_id, f"{LANGUAGE_SELECTION_PROMPT[lg]}", reply_markup=get_language_markup('flowlanguage'))
...
def select_node(message):
    ...
    keyboard = check_user_has_active_free_plan(user_id)
    bot.send_message(user_id, f"{NODE_TYPE_SELECTION_PROMPT[lg]}", reply_markup=keyboard)
...
def show_commands(message):
    ...
    bot.send_message(message.chat.id, f"{AVAILABLE_COMMANDS_PROMPT[lg]}\n{formatted_commands}", reply_markup=get_main_menu())
...
def get_profile_language(message):
    ...
    bot.send_message(user_id, f"{NICE_TO_MEET_YOU[lg]}!😊 {name}, "
                              f"{PROFILE_SETTING_PROMPT[lg]}⏳")
    bot.send_message(user_id, EMAIL_PROMPT[lg],
...
def get_email(message):
    ...
    if not validate_email(email):
        bot.send_message(user_id, INVALID_EMAIL_FORMAT[lg], reply_markup=get_force_reply())
        return
...
def get_mobile(message):
    ...
    if not validate_mobile(mobile):
        bot.send_message(user_id, INVALID_NUMBER_PROMPT[lg], reply_markup=get_force_reply())
        return
...
def handle_plan_selection(call):
    ...
    node_access = f"{FULL_NODE_ACCESS[lg]}" \
        if plan.call_transfer else f"{PARTIAL_NODE_ACCESS[lg]}"
    ...
    if plan.plan_price == 0:
        ...
        if set_subscription != f"{STATUS_CODE_200}":
            ...
            return
        bot.send_message(user_id, SUCCESSFUL_FREE_TRIAL_ACTIVATION[lg],
                         reply_markup=get_main_menu())
        return
...
def handle_auto_renewal_choice(call):
    ...
    if call.data == 'enable_auto_renewal_yes':
        ...
        auto_renewal_enabled = (f"{AUTO_RENEWAL_ENABLED[lg]}"
                                f" {PROCEED_PAYMENT[lg]}")
        bot.send_message(user_id, auto_renewal_enabled )
    else:
        auto_renewal_disabled = (f"{AUTO_RENEWAL_DISABLED[lg]} "
                                 f"{PROCEED_PAYMENT[lg]}")
...
def payment_through_wallet_balance(message):
    ...
    if response.status_code != 200:
        ...
        bot.send_message(user_id, f"{INSUFFICIENT_BALANCE[lg]}",
                         reply_markup=get_main_menu())
        return
...
def handle_payment_method(call):
    ...
    if currency_response != 200:
        bot.send_message(user_id, UNSUPPORTED_CURRENCY[lg] , reply_markup=get_main_menu())
        return
...
def handle_back_to_handle_payment(call):
    user_id = call.message.chat.id
    lg = get_user_language(user_id)
    bot.send_message(user_id, f"{SUBSCRIPTION_PAYMENT_METHOD_PROMPT[lg]}", reply_markup=get_currency_keyboard())
...
def make_crypto_payment(user_id, payment_method):
    ...
    if user_data[user_id]['transaction_type'] == 'top_up':
        bot.send_message(user_id, TOP_UP_AMOUNT_PROMPT[lg], reply_markup=get_force_reply())
    else:
        amount = user_data[user_id]['subscription_price']
...
def send_qr_code(user_id, address, qr_code_base64=None, ):
    ...
    with open("qr_code.png", "rb") as img_file:
        ...
        bot.send_message(user_id, f"{PART1_SCAN_PAYMENT_INFO[lg]} {balance_in_crypto:.4f} "
                                  ...
                                  f"{PART6_SCAN_PAYMENT_INFO[lg]}", reply_markup=get_main_menu())
    return
    ...
    if request.method == "POST":
        try:
            ...
            if plan_price > price_in_dollar:
                bot.send_message(user_id, f"{INSUFFICIENT_DEPOSIT_AMOUNT[lg]}\n"
            ...
            response = set_plan(user_id, plan_id, auto_renewal)
            if response['status'] != 200:
                bot.send_message(user_id, f"{TABLE_UPDATE_FAILED[lg]}\n"
                                          f"{response['message']}")
...
def get_webhook_data(request):
    ...
    message = (f"{TRANSACTION_ID[lg]} {transaction_id}\n"
    ...
    if auto_renewal == 'True':
        ...
        auto_renewal = False
    bot.send_message(user_id, f"{TRANSACTION_DETAILS[lg]}\n{message}", reply_markup=get_main_menu())
...
def create_flow(message):
    ...
    bot.send_message(user_id, ENTER_PATHWAY_NAME_PROMPT[lg], reply_markup=get_force_reply())
...
def add_node(message):
    ...
    bot.send_message(user_id, ENTER_CUSTOM_NODE_NAME[lg], reply_markup=get_force_reply())
...
def handle_pathway_details(call):
    ...
    pathway_info = (f"{NAME[lg]}: {pathway.get('name')}\n"
...
def view_flows(message):
    ...
    if filtered_pathways:
        ...
        markup.add(InlineKeyboardButton("Create IVR Flow ➕", callback_data="create_ivr_flow"))
        markup.add(InlineKeyboardButton("Back ↩️", callback_data="back"))
        bot.send_message(message.chat.id,NO_IVR_FLOW_AVAILABLE[lg] ,reply_markup=markup)
...
def handle_ask_name(message):
    ...
    bot.send_message(user_id, ENTER_PATHWAY_DESCRIPTION_PROMPT[lg], reply_markup=get_force_reply())
...
def handle_ask_description(message):
    ...
    if status_code == 200:
        ...
        bot.send_message(user_id, f"{LANGUAGE_SELECTION_FOR_FLOW[lg]}", reply_markup=get_language_markup('flowlanguage'))

    else:
        keyboard = check_user_has_active_free_plan(user_id)
        bot.send_message(user_id, f"{PROCESSING_ERROR[lg]} {response}!", reply_markup=keyboard)
...
def get_batch_call_base_prompt(message):
    ...
    if message.content_type == 'text':
        ...
        try:
            ...
            bot.send_message(user_id, f"{PROCESSING_ERROR[lg]} {str(e)}", reply_markup=get_main_menu())
    ...
    if calls_sent > max_contacts:
        bot.send_message(user_id,
                         ...
                         f"{ALLOWED_CONTACTS_PROMPT[lg]}", reply_markup=get_main_menu())
        return
    if calls_sent == 0:
        bot.send_message(user_id, SUBSCRIPTION_EXPIRED[lg], reply_markup=get_main_menu())
        return
    ...
    if response.status_code == 200:
        ...
        bot.send_message(user_id, f"{PROCESSING_ERROR[lg]} {response.text}", reply_markup=get_main_menu())
...
def handle_get_pathway(message):
    ...
    if status_code == 200:
        bot.send_message(user_id, FLOW_DELETED_SUCCESSFULLY[lg], reply_markup=get_main_menu())
    else:
        bot.send_message(user_id, f"{PROCESSING_ERROR[lg]} {response}!", reply_markup=get_main_menu())
...
def handle_pathway_selection(call):
    ...
    if step is None:
        ...
        bot.send_message(user_id, DELETE_FLOW_CONFIRMATION[lg],
        ...
        msg = (f"{ENTER_PHONE_NUMBER_PROMPT[lg]} {OR[lg]} "
               f"{UPLOAD_TXT[lg]}")
        bot.send_message(user_id, msg)
...
def handle_add_edges(message):
    ...
    if status != 200:
        bot.send_message(chat_id, f"{PROCESSING_ERROR[lg]} {response}", reply_markup=get_main_menu())
        return
...
def handle_target_node(call):
    ...
    bot.send_message(chat_id,ENTER_LABEL_PROMPT[lg],
                     reply_markup=get_force_reply())
...
def add_label(message):
    ...
    if response.status_code == 200:
        bot.send_message(chat_id, f"{EDGE_ADDED[lg]}\n"
...
def handle_add_node_id(message):
    ...
    if node == "Play Message ▶️":
        ...
        bot.send_message(user_id, ENTER_PHONE_NUMBER_FOR_CALL_TRANSFER[lg],
                         reply_markup=get_force_reply())
...
def get_menu(message):
    ...
    bot.send_message(user_id, ASSIGN_NUMBERS_FOR_MENU[lg],
                     reply_markup=get_force_reply())
...
def get_action_list(message):
    ...
    if response.status_code == 200:
        bot.send_message(user_id, f"'{node_name}' {NODE_ADDED[lg]} ✅",
...
def error_nodes_complete(message):
    user_id = message.chat.id
    lg = get_user_language(user_id)
    bot.send_message(user_id, SELECT_FROM_MENU[lg], reply_markup=get_node_complete_menu())
...
def text_to_speech(message):
    ...
    lg = get_user_language(user_id)
    text = message.text
    bot.send_message(user_id, USE_TEXT_TO_SPEECH_PROMPT[lg],
...
def error_message_input_type(message):
    ...
    bot.send_message(user_id, SELECTION_PROMPT[lg], reply_markup=get_play_message_input_type())
...
def handle_get_node_type(message):
    ...
    if node_type == 'Text-to-Speech 🗣️':
        ...
        bot.send_message(user_id, f"{ENTER_MESSAGE_PROMPT[lg]} {message_type}: ",
                         reply_markup=get_force_reply())
...
def handle_play_message(message):
    ...
    bot.send_message(user_id, SELECT_VOICE_TYPE_PROMPT[lg], reply_markup=get_voice_type_menu())
...
def handle_call_failure(message):
    ...
    lg = get_user_language(user_id)
    text = message.text
    bot.send_message(user_id, CALL_FAILURE_PROMPT[lg], reply_markup=get_call_failed_menu())
...
def transfer_to_agent(message):
    ...
    if phone_numbers:
        bot.send_message(user_id, PREVIOUSLY_ENTERED_NUMBERS[lg],
                         reply_markup=get_reply_keyboard(['Yes', 'No']))
        user_data[user_id] = {'step': 'use_previous_number', 'phone_numbers': list(phone_numbers)}
    else:
        bot.send_message(user_id, ENTER_PHONE_NUMBER_TO_TRANSFER[lg], reply_markup=get_force_reply())
...
def handle_use_previous_number(message):
    ...
    if text == 'Yes':
        ...
        bot.send_message(user_id, SELECT_PHONE_NUMBER[lg] ,
        ...
        bot.send_message(user_id, YES_OR_NO_PROMPT[lg] , reply_markup=get_reply_keyboard(['Yes', 'No']))
...
def handle_select_phone_number(call):
    ...
    bot.send_message(user_id, ADD_NODE_OR_DONE_PROMPT[lg],
...
def handle_enter_new_number(message):
    ...
    if validate_mobile(phone_number):
        ...
        bot.send_message(user_id, INVALID_PHONE_NUMBER[lg], reply_markup=get_force_reply())
...
def handle_add_or_done(message):
    ...
    if text == 'Add Another Node':
        ...
        bot.send_message(user_id, FINISHED_ADDING_NODES[lg], reply_markup=get_main_menu())
    else:
        bot.send_message(user_id, ADD_ANOTHER_OR_DONE_PROMPT[lg],
                         reply_markup=get_reply_keyboard(['Add Another Node', 'Done']))
...
def handle_single_ivr_call_flow(message):
    ...
    if validate_mobile(phone_number):
        ...
        if status == 200:
            bot.send_message(user_id, CALL_QUEUED_SUCCESSFULLY[lg],
                             reply_markup=get_main_menu())
            return
        bot.send_message(user_id, f"{PROCESSING_ERROR[lg]} {response}")
    else:
...
def handle_terms_and_conditions(message):
    ...
    bot.send_message(user_id, REVIEW_TERMS_AND_CONDITIONS[lg],
                     reply_markup=markup)
...
def handle_terms_response(call):
    ...
    if call.data == 'accept_terms':
        bot.send_message(user_id, f"✅ {SUCCESSFULLY_ACCEPTED_TERMS_AND_CONDITIONS[lg]}"
...
def handle_view_terms(call):
    user_id = call.from_user.id
    lg = get_user_language(user_id)
    bot.send_message(user_id, f"{VIEW_TERMS_AND_CONDITIONS[lg]} {TERMS_AND_CONDITIONS_URL}")
    handle_activate_subscription(call)
