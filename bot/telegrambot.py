...
from bot.models import Pathways, AudioFile, TransferCallNumbers
...
def handle_add_node_id(message):
    ...
    if text.isdigit() and 0 <= int(text) <= 9:
        ...
        if node == "Play Message ▶️":
            ...
            bot.send_message(user_id, "Please enter goodbye prompt for user:")
    else:
...
def handle_end_call_bot(message):
    ...
    print(text," ", node_name," ", node_text, " ",node_id, " ",pathway_id)

    bot.send_message(user_id, "in end call ")
