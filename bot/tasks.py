...
from TelegramBot.English import PROCESSING_ERROR, INSUFFICIENT_BALANCE_FOR_PAYMENT, WALLET_BALANCE, WALLET_TOP_UP, \
    ADDITIONAL_MINUTES, CHARGED_SUCCESSFULLY
...
def check_call_status():
    ...
    tracked_calls = BatchCallLogs.objects.filter(call_status__in=['new', 'queued', 'started'])
    ...
    for call in tracked_calls:
        ...
        if queue_status == 'started' and started_at:
            ...
            if duration_in_minutes > bulk_ivr_calls_left:
                ...
                remaining_minutes = float(bulk_ivr_calls_left) - float(duration_in_minutes)
                call_duration_record, created = CallDuration.objects.update_or_create(
                    call_id=call.call_id, pathway_id=call.pathway_id,
                    defaults={
                        'start_time': started_at, 'end_time': end_at, 'queue_status': queue_status,
