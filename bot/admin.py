...
class CallLogsTableAdmin(admin.ModelAdmin):
    ...
    search_fields = ("call_number", "user_id", "call_status", "created_at")
    list_filter = ("user_id", "call_status", "created_at")
...
class FeedbackLogsAdmin(admin.ModelAdmin):
    list_display = (
        "pathway_id",
    )  # Display pathway_id since feedback_questions is an array
...
class FeedbackDetailsAdmin(admin.ModelAdmin):
    list_display = (
        "call_id",
    )  # Display call_id since feedback_questions and answers are arrays
    search_fields = ("call_id",)
